          ...
          <div className="row justify-content-center">
            <div className="col-md-6 text-center">
              <div className="glass-effect rounded-4 p-5">
                ...
                <Link href="/groups" className="btn btn-warning btn-lg">
                  Kembali ke Grup
                </Link>
              </div>
            </div>
        ...
        <div className="row mb-4">
          <div className="col-12">
            <div className="d-flex justify-content-between align-items-center mb-4">
              <div>
                <Link href="/groups" className="btn btn-outline-warning btn-sm mb-3">
                  Kembali
                </Link>
              ...
              <div className="text-end">
                <div className="badge bg-warning text-dark fs-6 mb-2">
                  {isTeacher ? "👨‍🏫 Admin" : "👨‍🎓 Siswa"}
                </div>
                <div className="text-muted small">
                  <i className="fas fa-users me-1" />
                  {group.members.length} anggota •
                  <code className="text-light ms-1">{group.joinCode}</code>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-lg-3 col-md-4 mb-4">
            <div className="glass-effect rounded-4 p-3 h-100">
              <h6 className="text-warning mb-3">
                <i className="fas fa-users me-2" /> Anggota Grup
              </h6>
              <div className="members-list" style={{ maxHeight: 300, overflowY: "auto" }}>
                {group.members.map((m, idx) => (
                  <div
                    key={m.userId}
                    className="d-flex align-items-center p-2 bg-dark rounded-3 mb-2"
                  >
                    <div
                      className={`rounded-circle d-flex align-items-center justify-content-center me-3 ${m.role === "teacher" ? "bg-warning" : "bg-primary"}`}
                      style={{ width: 36, height: 36, minWidth: 36 }}
                    >
                      <i
                        className={`fas ${m.role === "teacher" ? "fa-user-tie" : "fa-user"} text-dark`}
                      />
                    </div>
                    <div className="flex-grow-1">
                      <div className="text-light small fw-bold text-truncate">
                        {m.username}
                      </div>
                      <div className="d-flex justify-content-between align-items-center">
                        <small className="text-muted">
                          {m.role === "teacher" ? "Admin" : "Siswa"}
                        </small>
                        {m.userId === user?.id && (
                          <span className="badge bg-success small">Anda</span>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-4 p-3 bg-secondary rounded-3">
                <h6 className="text-warning mb-2">
                  <i className="fas fa-info-circle me-2" /> Info Grup
                </h6>
                <div className="small">
                  ...
                  <div>
                    Kode: <code className="text-warning">{group.joinCode}</code>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="glass-effect rounded-4 h-100 d-flex flex-column" style={{ minHeight: 600 }}>
              <div className="p-3 border-bottom border-secondary d-flex justify-content-between align-items-center">
                <h5 className="text-warning mb-0">
                  <i className="fas fa-comments me-2" />Obrolan Grup
                </h5>
                          ...
                          <div className={`d-flex ${isOwn ? "justify-content-end" : "justify-content-start"}`}>
                            <div className="d-flex flex-column" style={{ maxWidth: "70%" }}>
                              <div className="mb-1 d-flex align-items-center">
                                ...
                                {msg.userId === group.teacherId && (
                                  <span className="badge bg-warning text-dark ms-1" title="Admin">Admin</span>
                                )}
                                {msg.userId === user?.id && (
                                  <span className="badge bg-success ms-1">Anda</span>
                                )}
                              </div>
                              <div className={`p-3 rounded-4 ${isOwn ? "bg-warning text-dark" : "bg-secondary text-light"}`}>
                                <div className="message-content">
                                  {msg.type === "code" ? (
                                    <pre className="bg-dark p-2 rounded text-light mb-1 small">
                                      <code>{msg.message}</code>
                                    </pre>
                                  ) : (
                                    <div className="mb-1" style={{ lineHeight: 1.4, whiteSpace: "pre-wrap" }}>{msg.message}</div>
                                  )}
                                </div>
              ...
              <div className="p-3 border-top border-secondary">
                ...
                <small className="text-muted mt-2">
                  <i className="fas fa-info-circle me-1" />Tekan Enter untuk mengirim pesan
                </small>
              </div>
            </div>
          </div>
